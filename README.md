...
- **성취가 쌓인다**: 주간 점 아래 달성률 고리, 🔥연속 기록, 지난 날짜로 가면 그날 성적과 회고.
  버리거나 미룬 것은 목록에서만 치워지고 **그날 점수에는 미완료로 남는다** — 안 한 걸 없던 일로 세면 동그라미가 거짓말을 하니까.
- **언제 할지**: 할 일을 눌러 시각을 정하면 시간표에 점선 블록으로 뜬다(마감과 다름 — "언제까지"가 아니라 "언제").
- **타이머와 연결**: 할 일의 ▶를 누르면 **스톱워치**가 그 할 일을 걸고 돌아간다(얼마나 걸릴지 모르고 시작하니까).
  멈추면 완료 여부를 묻고 걸린 시간을 기록한다. 카운트다운 타이머와 포모도로도 그대로 있다.
