...
export function composeReminder(user) {
  const date = todayStr();
  const tasks = db.prepare("SELECT title FROM tasks WHERE user = ? AND done = 0 AND due = ?").all(user, date);
...
export async function sendReminderTo(user, chatId) {
  const text = composeReminder(user);
