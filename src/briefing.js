...
export async function composeBriefing(user) {
  ...
  const todayTasks = await all('SELECT * FROM tasks WHERE "user" = ? AND done = 0 AND due = ?', [user, date]);
  const overdue = await all('SELECT * FROM tasks WHERE "user" = ? AND done = 0 AND due < ? ORDER BY due', [user, date]);
...
export async function sendBriefingTo(user, chatId) {
  return sendTelegram(await composeBriefing(user), chatId);
}
